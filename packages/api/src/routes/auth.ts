...
authRoutes.post('/merchant/login', async (c) => {
  const body = await c.req.json<{ store_code: string; pin: string }>();
  const { store_code, pin } = body;
  if (!store_code || !pin) {
    return c.json({ error: 'store_code and pin are required' }, 400);
  }
  const actor = await getActorByStoreCode(c.env.DB, store_code);
  return handleLogin(c, store_code, pin, actor, ActorType.MERCHANT);
});
